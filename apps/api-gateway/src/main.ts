...
class JourneysProxyController {
  ...
  @Get("operations") operations(@Headers("authorization") authorization: string, @Query("page") page?: string, @Query("pageSize") pageSize?: string, @Query("status") status?: string, @Query("search") search?: string) { const query = new URLSearchParams(); if (page) query.set("page", page); if (pageSize) query.set("pageSize", pageSize); if (status) query.set("status", status); if (search) query.set("search", search); return this.get(`/api/v1/journeys/operations${query.size ? `?${query.toString()}` : ""}`, authorization); }

...
export class PurchasesController {
  ...
  operations(@Query("page") page?: string, @Query("pageSize") pageSize?: string, @Query("status") status?: string) { const p = this.parsePage(page, "page", 1), s = this.parsePage(pageSize, "pageSize", 25); if (s > 100) throw new BadRequestException("pageSize must be between 1 and 100."); if (status !== undefined && !["PENDING", "PAID", "TICKET_ISSUED", "FAILED", "EXPIRED"].includes(status)) throw new BadRequestException("Invalid purchase status."); return this.purchases.listForOperations(p, s, status).then((data) => ({ success: true, data })); }
